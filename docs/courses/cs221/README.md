# CS221 Course Pack · Autumn 2021 · v0.1

Stanford CS221 (Artificial Intelligence: Principles and Techniques, Autumn 2021) human/agent co-learning package.

## Contents
- `CS221_interactive_Autumn2021_v0.1.html` — self-contained interactive course (runtime-rendered from embedded JSON, zero external references)
- `course_ir.json` — 9-week structure (objectives, pitfalls, quizzes, agent contracts)
- `concepts.json` — 32 canonical concepts (name ids, typed relations, non-empty weeks)
...
3. Re-embed both JSONs into the HTML `#cs221-data` block and regenerate the manifest.

## Version boundary
Autumn 2021 (archive_official). Lecture-topic labels derived from week themes are `official: false`; the canonical video source is the Stanford Online CS221 Autumn 2021 playlist.
